/*
 * $Header: $
 * This java source file is generated by pkliu on Tue Jan 30 14:38:15 CST 2018
 * For more information, please contact pkliu@sysfoundry.com
 */
package com.gateweb.orm.charge.entity;

import javax.persistence.*;
import java.io.Serializable;
import java.util.Objects;


/**
 *
 * @author pkliu
 *
 */
@Entity
@Table(name = "prepay_deduct_master")
public class PrepayDeductMasterEntity implements Serializable {

//long serialVersionUID jdk tool: serialver.exe 

	/**
	 * amount
	 */
	@Column(name = "amount")
	protected java.lang.Integer amount;
	
	/**
	 * company_id
	 */
	@Column(name = "company_id")
	protected java.lang.Integer companyId;
	
    /**
     * prepay_deduct_master_id java.lang.Integer , PK     * @GeneratedValue(strategy = GenerationType.AUTO)
     */
	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	@Column(name = "prepay_deduct_master_id", unique = true, nullable = false)
	protected java.lang.Integer prepayDeductMasterId;
	
	/**
	 * is_enable_over
	 */
	@Column(name = "is_enable_over")
	protected java.lang.String isEnableOver;
	
	/**
	 * creator_id
	 */
	@Column(name = "creator_id")
	protected java.lang.Integer creatorId;
	
	/**
	 * modifier_id
	 */
	@Column(name = "modifier_id")
	protected java.lang.Integer modifierId;
	
	/**
	 * create_date
	 */
	@Column(name = "create_date")
	protected java.sql.Timestamp createDate;
	
	/**
	 * modify_date
	 */
	@Column(name = "modify_date")
	protected java.sql.Timestamp modifyDate;
	



    
	/**
	 * 002
	 * @return java.lang.Integer amount
	 */
	public java.lang.Integer getAmount() {
		return this.amount;
	}
	
	/** 0001
	 * @param data Set the amount
	 */	
	public void setAmount(java.lang.Integer data) {
		this.amount = data;
	}	
	/**
	 * 002
	 * @return java.lang.Integer companyId
	 */
	public java.lang.Integer getCompanyId() {
		return this.companyId;
	}
	
	/** 0001
	 * @param data Set the companyId
	 */	
	public void setCompanyId(java.lang.Integer data) {
		this.companyId = data;
	}	
	/**
	 * 002
	 * @return java.lang.Integer prepayDeductMasterId
	 */
	public java.lang.Integer getPrepayDeductMasterId() {
		return this.prepayDeductMasterId;
	}
	
	/** 0001
	 * @param data Set the prepayDeductMasterId
	 */	
	public void setPrepayDeductMasterId(java.lang.Integer data) {
		this.prepayDeductMasterId = data;    //zzz
	}	
	/**
	 * 002
	 * @return java.lang.String isEnableOver
	 */
	public java.lang.String getIsEnableOver() {
		return this.isEnableOver;
	}
	
	/** 0001
	 * @param data Set the isEnableOver
	 */	
	public void setIsEnableOver(java.lang.String data) {
		this.isEnableOver = data;
	}	
	/**
	 * 002
	 * @return java.lang.Integer creatorId
	 */
	public java.lang.Integer getCreatorId() {
		return this.creatorId;
	}
	
	/** 0001
	 * @param data Set the creatorId
	 */	
	public void setCreatorId(java.lang.Integer data) {
		this.creatorId = data;
	}	
	/**
	 * 002
	 * @return java.lang.Integer modifierId
	 */
	public java.lang.Integer getModifierId() {
		return this.modifierId;
	}
	
	/** 0001
	 * @param data Set the modifierId
	 */	
	public void setModifierId(java.lang.Integer data) {
		this.modifierId = data;
	}	
	/**
	 * 002
	 * @return java.sql.Timestamp createDate
	 */
	public java.sql.Timestamp getCreateDate() {
		return this.createDate;
	}
	
	/** 0001
	 * @param data Set the createDate
	 */	
	public void setCreateDate(java.sql.Timestamp data) {
		this.createDate = data;
	}	
	/**
	 * 002
	 * @return java.sql.Timestamp modifyDate
	 */
	public java.sql.Timestamp getModifyDate() {
		return this.modifyDate;
	}
	
	/** 0001
	 * @param data Set the modifyDate
	 */	
	public void setModifyDate(java.sql.Timestamp data) {
		this.modifyDate = data;
	}	



	/**
	 *
	 */
	public PrepayDeductMasterEntity(){
	}

	/**
	 * full constructor 
	 * @param amount 
	 * @param companyId 
	 * @param prepayDeductMasterId 
	 * @param isEnableOver 
	 * @param creatorId 
	 * @param modifierId 
	 * @param createDate 
	 * @param modifyDate 
	 */
	public PrepayDeductMasterEntity (
		 java.lang.Integer amount 
		, java.lang.Integer companyId 
		, java.lang.Integer prepayDeductMasterId 
		, java.lang.String isEnableOver 
		, java.lang.Integer creatorId 
		, java.lang.Integer modifierId 
		, java.sql.Timestamp createDate 
		, java.sql.Timestamp modifyDate 
        ) {
		this.setAmount(amount);
		this.setCompanyId(companyId);
		this.setPrepayDeductMasterId(prepayDeductMasterId);
		this.setIsEnableOver(isEnableOver);
		this.setCreatorId(creatorId);
		this.setModifierId(modifierId);
		this.setCreateDate(createDate);
		this.setModifyDate(modifyDate);
    }

	@Override
	public int hashCode() {
		return Objects.hash(amount, companyId, prepayDeductMasterId, isEnableOver, creatorId, modifierId, createDate, modifyDate);
	}

	@Override
	public String toString() {
		return "PrepayDeductMasterEntity{" +
				"amount=" + amount +
				", companyId=" + companyId +
				", prepayDeductMasterId=" + prepayDeductMasterId +
				", isEnableOver='" + isEnableOver + '\'' +
				", creatorId=" + creatorId +
				", modifierId=" + modifierId +
				", createDate=" + createDate +
				", modifyDate=" + modifyDate +
				'}';
	}
	
	/**
	 * Indicates whether some other object is "equal to" this one.
	 * @return true is equal, false is not equal
	 */
	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;

		if (obj == null || !(obj instanceof PrepayDeductMasterEntity))
			return false;

		PrepayDeductMasterEntity key = (PrepayDeductMasterEntity) obj;
		if (this.prepayDeductMasterId != key.prepayDeductMasterId ) 
        	return false;

		return true;
    }

}