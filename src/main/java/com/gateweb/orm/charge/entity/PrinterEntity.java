/*
 * $Header: $
 * This java source file is generated by pkliu on Tue Jan 30 14:38:15 CST 2018
 * For more information, please contact pkliu@sysfoundry.com
 */
package com.gateweb.orm.charge.entity;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.Id;
import javax.persistence.Table;
import java.io.Serializable;
import java.util.Objects;


/**
 * @author pkliu
 */
@Entity
@Table(name = "printer")
public class PrinterEntity implements Serializable {

//long serialVersionUID jdk tool: serialver.exe 

    /**
     * company_type
     */
    @Column(name = "company_type")
    protected java.lang.Integer companyType;

    /**
     * company_id
     */
    @Column(name = "company_id")
    protected java.lang.Integer companyId;

    /**
     * com_port
     */
    @Column(name = "com_port")
    protected java.lang.Integer comPort;

    /**
     * take_number
     */
    @Column(name = "take_number")
    protected java.lang.Integer takeNumber;

    /**
     * printer_type
     */
    @Column(name = "printer_type")
    protected java.lang.Integer printerType;

    /**
     * encryption
     */
    @Column(name = "encryption")
    protected java.lang.Boolean encryption;

    /**
     * creator_id
     */
    @Column(name = "creator_id")
    protected java.lang.Integer creatorId;

    /**
     * modifier_id
     */
    @Column(name = "modifier_id")
    protected java.lang.Integer modifierId;

    /**
     * printer_id java.lang.Integer , PK     * @GeneratedValue(strategy = GenerationType.AUTO)
     */
    @Id
//	@GeneratedValue(strategy = GenerationType.IDENTITY)
    //移除直接指定id的限制
    @Column(name = "printer_id", unique = true, nullable = false)
    protected java.lang.Integer printerId;

    /**
     * printer_no
     */
    @Column(name = "printer_no")
    protected java.lang.String printerNo;

    /**
     * create_date
     */
    @Column(name = "create_date")
    protected java.sql.Timestamp createDate;

    /**
     * close
     */
    @Column(name = "close")
    protected java.lang.Boolean close;

    /**
     * modify_date
     */
    @Column(name = "modify_date")
    protected java.sql.Timestamp modifyDate;

    /**
     * key
     */
    @Column(name = "key")
    protected java.lang.String key;


    /**
     * 002
     *
     * @return java.lang.Integer companyType
     */
    public java.lang.Integer getCompanyType() {
        return this.companyType;
    }

    /**
     * 0001
     *
     * @param data Set the companyType
     */
    public void setCompanyType(java.lang.Integer data) {
        this.companyType = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer companyId
     */
    public java.lang.Integer getCompanyId() {
        return this.companyId;
    }

    /**
     * 0001
     *
     * @param data Set the companyId
     */
    public void setCompanyId(java.lang.Integer data) {
        this.companyId = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer comPort
     */
    public java.lang.Integer getComPort() {
        return this.comPort;
    }

    /**
     * 0001
     *
     * @param data Set the comPort
     */
    public void setComPort(java.lang.Integer data) {
        this.comPort = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer takeNumber
     */
    public java.lang.Integer getTakeNumber() {
        return this.takeNumber;
    }

    /**
     * 0001
     *
     * @param data Set the takeNumber
     */
    public void setTakeNumber(java.lang.Integer data) {
        this.takeNumber = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer printerType
     */
    public java.lang.Integer getPrinterType() {
        return this.printerType;
    }

    /**
     * 0001
     *
     * @param data Set the printerType
     */
    public void setPrinterType(java.lang.Integer data) {
        this.printerType = data;
    }

    /**
     * 002
     *
     * @return java.lang.Boolean encryption
     */
    public java.lang.Boolean getEncryption() {
        return this.encryption;
    }

    /**
     * 0001
     *
     * @param data Set the encryption
     */
    public void setEncryption(java.lang.Boolean data) {
        this.encryption = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer creatorId
     */
    public java.lang.Integer getCreatorId() {
        return this.creatorId;
    }

    /**
     * 0001
     *
     * @param data Set the creatorId
     */
    public void setCreatorId(java.lang.Integer data) {
        this.creatorId = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer modifierId
     */
    public java.lang.Integer getModifierId() {
        return this.modifierId;
    }

    /**
     * 0001
     *
     * @param data Set the modifierId
     */
    public void setModifierId(java.lang.Integer data) {
        this.modifierId = data;
    }

    /**
     * 002
     *
     * @return java.lang.Integer printerId
     */
    public java.lang.Integer getPrinterId() {
        return this.printerId;
    }

    /**
     * 0001
     *
     * @param data Set the printerId
     */
    public void setPrinterId(java.lang.Integer data) {
        this.printerId = data;    //zzz
    }

    /**
     * 002
     *
     * @return java.lang.String printerNo
     */
    public java.lang.String getPrinterNo() {
        return this.printerNo;
    }

    /**
     * 0001
     *
     * @param data Set the printerNo
     */
    public void setPrinterNo(java.lang.String data) {
        this.printerNo = data;
    }

    /**
     * 002
     *
     * @return java.sql.Timestamp createDate
     */
    public java.sql.Timestamp getCreateDate() {
        return this.createDate;
    }

    /**
     * 0001
     *
     * @param data Set the createDate
     */
    public void setCreateDate(java.sql.Timestamp data) {
        this.createDate = data;
    }

    /**
     * 002
     *
     * @return java.lang.Boolean close
     */
    public java.lang.Boolean getClose() {
        return this.close;
    }

    /**
     * 0001
     *
     * @param data Set the close
     */
    public void setClose(java.lang.Boolean data) {
        this.close = data;
    }

    /**
     * 002
     *
     * @return java.sql.Timestamp modifyDate
     */
    public java.sql.Timestamp getModifyDate() {
        return this.modifyDate;
    }

    /**
     * 0001
     *
     * @param data Set the modifyDate
     */
    public void setModifyDate(java.sql.Timestamp data) {
        this.modifyDate = data;
    }

    /**
     * 002
     *
     * @return java.lang.String key
     */
    public java.lang.String getKey() {
        return this.key;
    }

    /**
     * 0001
     *
     * @param data Set the key
     */
    public void setKey(java.lang.String data) {
        this.key = data;
    }


    /**
     *
     */
    public PrinterEntity() {
    }

    /**
     * full constructor
     *
     * @param companyType
     * @param companyId
     * @param comPort
     * @param takeNumber
     * @param printerType
     * @param encryption
     * @param creatorId
     * @param modifierId
     * @param printerId
     * @param printerNo
     * @param createDate
     * @param close
     * @param modifyDate
     * @param key
     */
    public PrinterEntity(
            java.lang.Integer companyType
            , java.lang.Integer companyId
            , java.lang.Integer comPort
            , java.lang.Integer takeNumber
            , java.lang.Integer printerType
            , java.lang.Boolean encryption
            , java.lang.Integer creatorId
            , java.lang.Integer modifierId
            , java.lang.Integer printerId
            , java.lang.String printerNo
            , java.sql.Timestamp createDate
            , java.lang.Boolean close
            , java.sql.Timestamp modifyDate
            , java.lang.String key
    ) {
        this.setCompanyType(companyType);
        this.setCompanyId(companyId);
        this.setComPort(comPort);
        this.setTakeNumber(takeNumber);
        this.setPrinterType(printerType);
        this.setEncryption(encryption);
        this.setCreatorId(creatorId);
        this.setModifierId(modifierId);
        this.setPrinterId(printerId);
        this.setPrinterNo(printerNo);
        this.setCreateDate(createDate);
        this.setClose(close);
        this.setModifyDate(modifyDate);
        this.setKey(key);
    }

    @Override
    public int hashCode() {
        return Objects.hash(companyType, companyId, comPort, takeNumber, printerType, encryption, creatorId, modifierId, printerId, printerNo, createDate, close, modifyDate, key);
    }

    @Override
    public String toString() {
        return "PrinterEntity{" +
                "companyType=" + companyType +
                ", companyId=" + companyId +
                ", comPort=" + comPort +
                ", takeNumber=" + takeNumber +
                ", printerType=" + printerType +
                ", encryption=" + encryption +
                ", creatorId=" + creatorId +
                ", modifierId=" + modifierId +
                ", printerId=" + printerId +
                ", printerNo='" + printerNo + '\'' +
                ", createDate=" + createDate +
                ", close=" + close +
                ", modifyDate=" + modifyDate +
                ", key='" + key + '\'' +
                '}';
    }

	/*
    @Override
	public boolean equals(Object object) {
		if (object == null || !(object instanceof PrinterEntity))
			return false;
		EqualsBuilder builder = new EqualsBuilder();
		builder.append(this.id, ((PrinterEntity) object).id);
		return builder.isEquals();
	}*/

    /**
     * Indicates whether some other object is "equal to" this one.
     *
     * @return true is equal, false is not equal
     */
    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;

        if (obj == null || !(obj instanceof PrinterEntity))
            return false;

        PrinterEntity key = (PrinterEntity) obj;
        if (this.printerId != key.printerId)
            return false;

        return true;
    }

}