...
//        getRoleCompany(whereSb,parameters,"cp.");
    ...
    public List<Company> getCompanyList(Boolean parent) throws Exception {
        String sql = "SELECT * FROM company WHERE company_type = 1 or company_type=4";
        if (parent.equals(true)) {
            sql += " AND parent_id is null ";
        }
        return createQueryForList(sql, null, Company.class);
    }
    ...
    public List<Company> getCompanyListByRole() throws Exception {
        List parameters = new ArrayList();
        StringBuffer sql = new StringBuffer();
        sql.append("SELECT * FROM company cp WHERE company_type = 1  ");
//        getRoleCompany(sql,parameters,"cp.");
        return createQueryForList(sql.toString(), parameters, Company.class);
    }
